...
def main(
    ...
    base_url: Optional[str] = typer.Option(
        None,
        help="Переопределить основной Makaba URL (например https://2ch.org/makaba/makaba.fcgi)",
    ),
    extra_base_url: Optional[List[str]] = typer.Option(
        None,
        help="Добавить дополнительный URL для попытки запроса (можно указать несколько раз)",
    ),
    ...
    if html_file:
        ...
        console.print(f"Скачиваем /{board}/{thread}...")
        client = DvachClient(
            base_url=base_url,
            cookie=cookie,
            user_agent=user_agent,
            extra_base_urls=extra_base_url or [],
        )
        try:
