...
class AnimeChatbot:
    ...
    def _generate(self, prompt: str) -> str:
        ...
        outputs = self.generator(prompt, **common_kwargs)
        generated = outputs[0]["generated_text"]
        if not generated.strip():
            retry_kwargs = dict(common_kwargs)
            retry_kwargs["temperature"] = min(gen_cfg.temperature + 0.15, 1.3)
            retry_kwargs["top_p"] = min(gen_cfg.top_p + 0.05, 0.99)
            outputs = self.generator(prompt, **retry_kwargs)
            generated = outputs[0]["generated_text"]
        return generated
